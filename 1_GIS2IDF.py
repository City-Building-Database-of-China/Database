...
def initialize_idf_environment(version, epw_path):
    # Resolve EnergyPlus via config.ENERGYPLUS_DIR / default install paths.
    from config import (
        energyplus_minimal_idf,
        energyplus_version_updater_idd,
        require_energyplus_root,
    )

    ep_root = require_energyplus_root()
    IDF.setiddname(str(energyplus_version_updater_idd(ep_root)))
    idf = IDF(str(energyplus_minimal_idf(ep_root)))
    idf.epw = epw_path
    return idf
...
if __name__ == "__main__":
    # Paths are relative to the repository (see config.py).
    # Use Prototype GIS for replication (not CityBuilding full-city zips).
    from config import (
        AGE_DATA_DIR,
        ENERGYPLUS_VERSION,
        NANJING_EPW,
        NANJING_PROTOTYPE_SHP,
        NON_GEOMETRY_XLSX,
        READY_IDF_DIR,
        REPO_ROOT,
    )
    ...
    shpfile_path = str(NANJING_PROTOTYPE_SHP)
    epw_path = str(NANJING_EPW)
    non_geometry_data_path = str(NON_GEOMETRY_XLSX)
    age_data_folder = str(AGE_DATA_DIR)
    save_folder = str(READY_IDF_DIR / "320100NANJINGSHI")

    version = ENERGYPLUS_VERSION
    floor_height = 3

    print("Starting IDF generation.")
    print(f"  shapefile: {shpfile_path}")
